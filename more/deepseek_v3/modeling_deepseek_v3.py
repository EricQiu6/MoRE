...
#
# This file is derived from the HuggingFace Transformers DeepSeek-v3
# implementation, which is based on the DeepSeek AI team's DeepSeek-V3
...
try:
    ...
    def merge_with_config_defaults(f):
        return f  # no-op for older transformers
try:
    from ...utils.output_capturing import capture_outputs
except ImportError:
    def capture_outputs(f):
        return f  # no-op for older transformers
...
class DeepseekV3MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Per-token top-k expert ids for this forward (used by moe_routing_viz cross-layer stats).
        self._routing_topk_indices = topk_indices.detach()

        # Aggregate routing statistics for external analysis (Sonic-style).
        # Shapes: topk_indices/topk_weights: (num_tokens, top_k)
        with torch.no_grad():
            flat_indices = topk_indices.reshape(-1)
            flat_weights = topk_weights.reshape(-1)
            num_experts = self.config.num_local_experts

            # Expert token counts (each token contributes once per selected expert)
            expert_counts = torch.bincount(flat_indices, minlength=num_experts)

            # Aggregate routing probabilities per expert
            expert_acc_probs = torch.zeros(
                num_experts,
                device=router_logits.device,
                dtype=flat_weights.dtype,
            )
            expert_acc_probs.scatter_add_(0, flat_indices, flat_weights)

            # Match SonicMoE attribute names expected by moe_routing_viz.py
            self._routing_expert_freq = expert_counts
            self._routing_acc_probs = expert_acc_probs
...
class DeepseekV3Model(DeepseekV3PreTrainedModel):
    def __init__(self, config: DeepseekV3Config):
        ...
        self.use_depth_embedding = getattr(config, "use_depth_embedding", False)
    ...
    def tie_weights(self, *args, **kwargs):
        ...
        config = self.config
        grouping_factor = getattr(config, "grouping_factor", 1)
        if grouping_factor <= 1:
            return
        tying_strategy = getattr(config, "tying_strategy", "block")
        first_k = config.first_k_dense_replace
        num_moe_layers = config.num_hidden_layers - first_k
        if num_moe_layers <= 0:
            return
        num_unique_sets = num_moe_layers // grouping_factor
        tied_map = {}
        for layer_idx in range(config.num_hidden_layers):
            if layer_idx < first_k:
                continue
            moe_idx = layer_idx - first_k
            if tying_strategy == "block":
                leader_moe_idx = (moe_idx // grouping_factor) * grouping_factor
            elif tying_strategy == "strided":
                leader_moe_idx = moe_idx % num_unique_sets
            else:
                raise ValueError(f"Unknown tying_strategy: {tying_strategy}")
            leader_layer_idx = first_k + leader_moe_idx
            if layer_idx == leader_layer_idx:
                continue
            leader_moe = self.layers[leader_layer_idx].mlp
            curr_moe = self.layers[layer_idx].mlp
            curr_moe.experts.gate_up_proj = leader_moe.experts.gate_up_proj
            curr_moe.experts.down_proj = leader_moe.experts.down_proj
            tied_map[f"layers.{layer_idx}.mlp.experts.gate_up_proj"] = f"layers.{leader_layer_idx}.mlp.experts.gate_up_proj"
            tied_map[f"layers.{layer_idx}.mlp.experts.down_proj"] = f"layers.{leader_layer_idx}.mlp.experts.down_proj"
            curr_moe.shared_experts.gate_proj.weight = leader_moe.shared_experts.gate_proj.weight
            curr_moe.shared_experts.up_proj.weight = leader_moe.shared_experts.up_proj.weight
            curr_moe.shared_experts.down_proj.weight = leader_moe.shared_experts.down_proj.weight
            tied_map[f"layers.{layer_idx}.mlp.shared_experts.gate_proj.weight"] = f"layers.{leader_layer_idx}.mlp.shared_experts.gate_proj.weight"
            tied_map[f"layers.{layer_idx}.mlp.shared_experts.up_proj.weight"] = f"layers.{leader_layer_idx}.mlp.shared_experts.up_proj.weight"
            tied_map[f"layers.{layer_idx}.mlp.shared_experts.down_proj.weight"] = f"layers.{leader_layer_idx}.mlp.shared_experts.down_proj.weight"
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            depth_emb = None
            if self.all_depth_embeddings is not None:
