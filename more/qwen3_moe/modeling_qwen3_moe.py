...
try:
    ...
    def capture_outputs(f):
        return f
    def OutputRecorder(*args, **kwargs):
        return None
...
class Qwen3MoeModel(Qwen3MoePreTrainedModel):
    ...
    def tie_weights(self, *args, **kwargs):
        ...
        moe_layer_indices = [
            ...
            and (config.num_experts > 0 and (i + 1) % config.decoder_sparse_step == 0)
        ]
        if not moe_layer_indices:
            return
        tying_strategy = getattr(config, "tying_strategy", "block")
        num_unique_sets = len(moe_layer_indices) // grouping_factor
        if num_unique_sets <= 0:
            return
        tied_map = {}
        for moe_idx, layer_idx in enumerate(moe_layer_indices):
            ...
            leader_layer_idx = moe_layer_indices[leader_moe_idx]
            ...
            tied_map[f"layers.{layer_idx}.mlp.experts.gate_up_proj"] = (
                f"layers.{leader_layer_idx}.mlp.experts.gate_up_proj"
            )
            tied_map[f"layers.{layer_idx}.mlp.experts.down_proj"] = (
                f"layers.{leader_layer_idx}.mlp.experts.down_proj"
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            depth_emb = (
                self.all_depth_embeddings[layer_idx] if self.all_depth_embeddings is not None else None
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if output_router_logits:
            ...
            # load_balancing_loss_func may return 0 (int) when there are no logits;
            # only add the aux term when we have a tensor-like loss.
            if labels is not None and hasattr(aux_loss, "to"):
